...
class APIHandler:
        ...
        if not os.path.exists(APP_CONFIG_FILE):
            ...
            with open(APP_CONFIG_FILE, 'w') as configfile:
                ...
                with open(APP_CONFIG_FILE, 'r+') as f:
                    content = f.read()
                    f.seek(0, 0)
                    f.write("# --- Movie Explorer Configuration ---\n\n"
    ...
    def reload_yts_domains(self):
        print("Reloading YTS domains from file...")
        self.yts_domains = self._load_yts_domains()
        self.yts_active_domain = None

    def reload_app_config(self):
        """Public method to allow the main app to trigger a reload of the app config."""
        print("Reloading app configuration (API keys)...")
        self._load_app_config()

    def _find_yts_active_domain(self):
        print("Searching for an active YTS domain...")
        for domain in self.yts_domains:
            ...
            try:
                ...
                if response.status_code == 200 and response.json().get('status') == 'ok':
                    ...
                    print(f"Active YTS domain set: {self.yts_active_domain}")
                    return domain
    ...
    def _make_yts_request(self, endpoint, params=None):
        if not self.yts_active_domain:
            if not self._find_yts_active_domain():
                raise ConnectionError("No active YTS domains found. Check yts_domains.json.")
        ...
        try:
            ...
            else: raise Exception(data.get('status_message', 'Unknown YTS API error'))
        except requests.RequestException as e:
            self.yts_active_domain = None
            raise ConnectionError(f"YTS network request failed: {e}") from e

    def list_movies(self, **kwargs):
        params = {'limit': 50}; params.update(kwargs)
    ...
    def get_tmdb_details(self, imdb_id):
        ...
        try:
            find_url = f"https://api.themoviedb.org/3/find/{imdb_id}"; params = {'api_key': self.tmdb_api_key, 'external_source': 'imdb_id'}
            response = requests.get(find_url, params=params, timeout=5); response.raise_for_status(); find_data = response.json()
            if not find_data.get('movie_results'): return None
            tmdb_id = find_data['movie_results'][0]['id']
            details_url = f"https://api.themoviedb.org/3/movie/{tmdb_id}"; params = {'api_key': self.tmdb_api_key, 'append_to_response': 'videos,credits'}
            response = requests.get(details_url, params=params, timeout=5); response.raise_for_status(); details_data = response.json()
            enhanced_data = {}
    ...
    def get_image_data(self, url):
        try:
            with urlopen(url, timeout=10) as response: return response.read()
        except Exception as e:
